...
def train_epoch(epoch, wandb):
    ...
    # 每隔指定epoch保存一次权重
    if ((epoch + 1) % args.save_epoch_interval == 0) and (not ddp or dist.get_rank() == 0):
        model.eval()
        # 保存当前epoch的权重，命名中包含epoch编号以避免覆盖
        lora_save_path = f'{args.save_dir}/lora/{args.lora_name}_{lm_config.hidden_size}_epoch_{epoch+1}.pth'
        os.makedirs(os.path.dirname(lora_save_path), exist_ok=True)
        Logger(f'Saving checkpoint at epoch {epoch+1} to {lora_save_path}')
        save_lora(model, lora_save_path)
        model.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_epoch_interval", type=int, default=10, help="每隔多少个epoch保存一次lora权重")
