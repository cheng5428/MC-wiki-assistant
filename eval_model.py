...
from scripts.check_repetition import RepetitionCheckingStreamer
import re
        ...
        if args.lora_name == 'None':
            ...
            lora_prompt_datas = {
                ...
                'lora_mc': [
                    ...
                    '钻石如何获取',
                    'Minecraft 中矿车的用途是什么',
                    'Minecraft 中的石头和基岩的声音有什么区别',
                    '与其他版本的 Minecraft 相比，基岩版中工作台的制作配方的主要区别是什么？',
                    '锄头工具在 Minecraft 中的主要用途是什么',
                    '如何获取煤炭？',
                ]
            }
            # 对于以lora_mc开头的LoRA名称，使用统一的问题集
            if args.lora_name.startswith('lora_mc'):
                prompt_datas = lora_prompt_datas['lora_mc']
            else:
...
def main():
    ...
    parser.add_argument('--max_seq_len', default=500, type=int) # default=8192
    ...
    streamer = RepetitionCheckingStreamer(tokenizer, skip_prompt=True, skip_special_tokens=True)

    messages = []
    for idx, prompt in enumerate(prompts if test_mode == 0 else iter(lambda: input('👶: '), '')):
        # setup_seed(random.randint(0, 2048))
        setup_seed(26)  # 如需固定每次输出则换成【固定】的随机种子
        ...
        streamer.reset()  # 重置streamer状态
        print('🤖️: ', end='')
        generated_ids = model.generate(
            inputs["input_ids"],
            max_new_tokens=args.max_seq_len,
            early_stopping=True,  # 当模型生成EOS时提前停止
            #################
